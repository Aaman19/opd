        ...
        <div class="table-responsive">
            <table class="table table-bodererd">
              ...
              <tbody>
                ...
                // for reteriving diagnosis info
                $query1 = "SELECT * FROM diagnose WHERE p_id = '$id' ORDER BY opd_id DESC ";
                ...
                if(mysqli_num_rows($query_run) > 0){
                 ...
                 $opd_id = $row1['opd_id'];
                 $do_id = $row1['d_id'];
                 $timestamp = $row1['diagnosis_date'];
                 // $splits =  explode(" ",$timestamp);
                 $get_date = date('Y-m-d',strtotime($timestamp));
                 $get_time = date('H:i:s',strtotime($timestamp));

                 // for reteriving Doctor's info
                 $query2 = "SELECT * FROM doctor_info WHERE d_id = '$do_id' ";
                 $query_run2 = mysqli_query($con,$query2);
                 $row2 = mysqli_fetch_array($query_run2);
                 $d_name = $row2['d_name'];
                ?>
                <tr>
                  <td><?php echo $row1['opd_id'] ?></td>
                  <td><?php echo $get_date ?></td>
                  <td><?php echo $get_time ?></td>
                  <td><?php echo $do_id ?></td>
                  <td><?php echo "Dr ".$d_name ?></td>
                  <td><a href="medication.php?id=<?php echo $id; ?>&d_id=<?php echo $do_id; ?>&opd_id=<?php echo $opd_id; ?>"><i class="fas fa-eye" style="font-size:20px;color:DodgerBlue;text-shadow:2px 2px 4px #000000;"></i></a></td>
                  <td><span  data-bs-toggle="modal" data-bs-target="#deleteModal"><i class="fas fa-trash-alt" style="font-size:20px;color:red;text-shadow:2px 2px 4px #000000;"data-bs-toggle="tooltip" data-bs-placement="bottom" title="Delete"></i></span></td>
                </tr>
                <?php
                  }
